...
const sizeMap = {
  sm: { icon: 'size-7', pad: 'p-1', text: 'text-lg' },
  md: { icon: 'size-8', pad: 'p-1', text: 'text-xl' },
  lg: { icon: 'size-12', pad: 'p-1.5', text: 'text-2xl' },
};

/**
 * Material Design Icons — sheep (Pictogrammers, Apache-2.0)
 * https://pictogrammers.com/library/mdi/icon/sheep/
 */
    ...
    <>
      <div
        className={cn(
          s.icon,
          s.pad,
          'shrink-0 overflow-hidden rounded-[22%] bg-laranja text-white shadow-sm'
        )}
      >
        <LogoIcon />
      </div>
      {showText && (
        <span
          className={cn(
            s.text,
            'font-display font-bold text-tinta leading-tight tracking-tight whitespace-nowrap'
          )}
        >
          Pequenos Discípulos
        </span>
  ...
  if (href) {
    return (
      <Link
        ...
        onClick={onClick}
        className={cn(
          'inline-flex items-center gap-3 rounded-livro transition-opacity hover:opacity-80',
